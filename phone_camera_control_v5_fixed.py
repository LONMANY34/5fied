bl_info = {
    "name": "Phone Camera Control v5fix",
    "author": "Custom",
    "version": (5, 1, 0),
    "blender": (3, 0, 0),
...
# Pose buffer — เก็บ pose ระหว่าง record แล้ว bake ตอน Stop
# วิธีนี้ไม่แตะ Blender keyframe เลยระหว่าง record → กล้องขยับอิสระ 100%
...
# ── Bake poses → F-Curve (ไม่ผ่าน keyframe_insert) ──

def _bake_to_fcurves(obj, poses):
    """เขียน pose buffer ลง F-Curve โดยตรง — ไม่ evaluate scene เลย"""
    if not poses:
        print("[PhoneCam] No poses to bake")
        return 0
    ...
    # ลบ action เก่า สร้างใหม่สะอาด
    if obj.animation_data.action:
        old = obj.animation_data.action
        obj.animation_data.action = None
        bpy.data.actions.remove(old)

    action = bpy.data.actions.new(name="PhoneCamRecord")
    obj.animation_data.action = action

    # สร้าง F-Curves และ pre-allocate keyframe points
    defs = [
    ...
    # เขียนค่าลง F-Curve ด้วย foreach_set (เร็วที่สุด)
    for ch_dp, ch_idx in defs:
        fc = fcs[(ch_dp, ch_idx)]
        pts = []
        for frame, loc, quat in poses:
            if ch_dp == "location":
                val = [loc.x, loc.y, loc.z][ch_idx]
            else:
                val = [quat.w, quat.x, quat.y, quat.z][ch_idx]
            pts.append(float(frame))
            pts.append(float(val))
        # foreach_set เขียนทุก co พร้อมกัน
    ...
    print(f"[PhoneCam] ✅ Baked {len(poses)} frames → '{action.name}'")
    return len(poses)

# ══════════════════════════════════════════════
#  TIMER 1 — ROTATION ONLY (60 fps)
#  - apply gyro → camera (ทุก tick ไม่ว่าจะ record หรือเปล่า)
#  - sample pose ลง buffer ถ้ากำลัง record
#  - ไม่แตะ keyframe / frame_set / frame_current เลย
# ══════════════════════════════════════════════
...
# ══════════════════════════════════════════════
#  TIMER 2 — ไม่ทำงาน (ลบออก แต่ยังต้อง register)
#  ใน v5fix เราใช้ pose buffer แทน timer keyframe
#  timer นี้เหลือแค่ dummy เพื่อ compatibility
# ══════════════════════════════════════════════

# (ลบ _timer_keyframe ออกแล้ว ใช้ pose buffer แทน)

_rot_timer_registered = False

def register_timers():
    global _rot_timer_registered
...
class PHONECAM_OT_StartRec(bpy.types.Operator):
    ...
    def execute(self, context):
        global _is_recording, _record_fps, _record_frame
        global _record_start_frame, _last_sample_time
        p = context.scene.phone_cam_props
        ...
        self.report({"INFO"}, f"⏺ REC {_record_fps}fps — ขยับกล้องได้เลย!")
        return {"FINISHED"}
...
class PHONECAM_OT_StopRec(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        print(f"[PhoneCam] ⏹ Stop — {len(poses)} poses collected")

        if not poses:
            self.report({"ERROR"}, "ไม่มี pose data! ลอง Record อีกครั้ง")
            return {"FINISHED"}

        obj = _get_obj()
        if not obj:
            self.report({"ERROR"}, "ไม่พบ Camera")
            return {"FINISHED"}

        # Bake ทั้งหมดลง F-Curve
        n = _bake_to_fcurves(obj, poses)
        p.rec_frame_count = n

        # ตั้ง scene range
        context.scene.frame_start   = 1
        context.scene.frame_end     = n + 2
        context.scene.frame_current = 1

        self.report({"INFO"}, f"✅ Baked {n} frames — กด Space เพื่อ Play!")
        return {"FINISHED"}

# ── Panel ──────────────────────────────────────

class PHONECAM_PT_Panel(bpy.types.Panel):
    bl_label = "📱 Phone Camera v5fix"
    ...
    def draw(self, context):
        ...
        if not p.is_recording:
            ...
            row2.operator("phonecam.stop_rec",
                          ...
                          icon="SNAP_FACE")
            rb.label(text="กล้องขยับได้อิสระ ✓", icon="INFO")
